  ...
  <div class="fade-in bg-slate-50 min-h-screen pb-40"> <!-- Main Background -->
            ...
            <div class="lg:col-span-8 space-y-8">
                ...
                <div class="relative pl-8 sm:pl-0">
                    ...
                    <div v-for="(reply, index) in ticket.replies" :key="reply.id" 
                         class="group relative mb-8 flex transition-all duration-500 ease-out pl-24"
    ...
    <Teleport to="body">
        <div v-if="ticket && ticket.status !== 'closed'" 
             class="fixed bottom-6 left-0 lg:left-[250px] right-0 z-[9999] flex justify-center pointer-events-none px-4"
             style="position: fixed !important; bottom: 1.5rem !important; right: 0 !important; z-index: 9999 !important; display: flex !important; justify-content: center !important; pointer-events: none !important;">
            ...
            <div class="w-full max-w-4xl bg-white/95 backdrop-blur-xl border border-gray-200 shadow-2xl rounded-2xl p-2 pointer-events-auto transform transition-all duration-300 pointer-events-auto"
                ...
                <form @submit.prevent="submitReply" class="flex gap-2 items-center" style="display: flex; gap: 0.5rem; align-items: center;">
                    <div class="flex-1 relative" style="flex: 1; position: relative;">
                        <textarea v-model="replyMessage" 
                                  class="w-full bg-gray-50 hover:bg-white focus:bg-white border-0 rounded-xl px-4 py-3 min-h-[52px] max-h-[200px] resize-none focus:ring-2 focus:ring-indigo-500 text-gray-700 placeholder:text-gray-400 transition-all font-medium block !m-0"
                                  rows="1" 
                                  @input="autoResize"
                                  style="width: 100%; background-color: #f8fafc; border-radius: 0.75rem; padding: 0.75rem 1rem; min-height: 52px; max-height: 200px; resize: none; border: 1px solid #e2e8f0; outline: none; margin: 0; display: block;"
                                  required 
                                  placeholder="Type your reply here..."></textarea>
                    </div>
                    <button type="submit" 
                            class="h-[52px] px-6 bg-slate-900 hover:bg-indigo-600 text-white rounded-xl font-bold shadow-md transition-all flex items-center gap-2 group disabled:opacity-50 disabled:cursor-not-allowed !m-0"
                            style="height: 52px; padding: 0 1.5rem; background-color: #0f172a; color: white; border-radius: 0.75rem; font-weight: 700; display: flex; align-items: center; gap: 0.5rem; border: none; cursor: pointer; margin: 0;"
    ...
    <Teleport to="body">
        <div v-if="showResolveModal" class="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/20 backdrop-blur-sm transition-opacity">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 transform scale-100 transition-all border border-slate-100">
                <div class="text-center">
                    <div class="w-16 h-16 bg-rose-50 rounded-full flex items-center justify-center mx-auto mb-4 text-rose-500 text-3xl">
                        &check;
                    </div>
                    <h3 class="text-xl font-bold text-slate-900 mb-2">Mark as Resolved?</h3>
                    <p class="text-slate-500 mb-6 text-sm leading-relaxed">
                        This will close the ticket and prevent further replies. You can reopen it later if needed.
                    </p>
                    <div class="flex gap-3">
                        <button @click="showResolveModal = false" class="flex-1 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl transition-colors">
                            Cancel
                        </button>
                        <button @click="confirmClose" class="flex-1 px-4 py-2.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl shadow-lg shadow-rose-200 transition-all">
                            Yes, Resolve
                        </button>
                    </div>
                </div>
...
onMounted(() => {
    console.log('Ticket Detail Premium UI Mounted - VERSION: NO_SVGS_NUCLEAR_FIX')
    fetchTicket()
})
</script>
